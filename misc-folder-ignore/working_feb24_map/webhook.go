...
func ProcessDeployment(nameSpace string, deploymentName string, serviceInstanceNum int, flow string) (map[string]string, bool) {
	...
	if IsCustomSchedulingDefined, ok := deploymentAnnotations["UseCustomKubeScheduler"]; ok && IsCustomSchedulingDefined == "true" {
		...
		glog.Infof("flow=%s serviceInstanceNum=%d Found a deployment %s with total replicas %d and strategy=%s", flow, serviceInstanceNum, deploymentName, numOfReplicas, strategy)
		//glog.Infof("\nnumOfReplicas=%d\n", numOfReplicas)
		if customSchedulingStrategyMap, ok := GetPodsCustomSchedulingStrategyMap(strategy, numOfReplicas, serviceInstanceNum); ok {
			glog.Infof("flow=%s serviceInstanceNum=%d customSchedulingStrategyMap=%v", flow, serviceInstanceNum, customSchedulingStrategyMap)
			for nodeLabel, replicas := range customSchedulingStrategyMap {
				glog.Infof("flow=%s serviceInstanceNum=%d nodeLabel=%s needs %d replicas\n", flow, serviceInstanceNum, nodeLabel, replicas)
				ExistingPodsList, result := GetNumOfExistingPods(nameSpace, deploymentName, nodeLabel, serviceInstanceNum)
				numOfExistingPods := len(ExistingPodsList)
				if result {
					glog.Infof("flow=%s serviceInstanceNum=%d nodeLabel=%s currently runs %d pods", flow, serviceInstanceNum, nodeLabel, numOfExistingPods)
					if numOfExistingPods < replicas {
						if flow == "CREATE" {
							glog.Infof("flow=%s serviceInstanceNum=%d Currently running %d pods is less than expected %d, scheduling pod on nodeLabel %s", flow, serviceInstanceNum, numOfExistingPods, replicas, nodeLabel)
						...
						glog.Infof("flow=%s serviceInstanceNum=%d Currently running %d pods is SAME as expected %d, ignoring the nodeLabel %s", flow, serviceInstanceNum, numOfExistingPods, replicas, nodeLabel)
					} else {
						if flow == "DELETE" {
							numOfPodsToBeDeleted := numOfExistingPods - replicas
							glog.Infof("flow=%s serviceInstanceNum=%d Currently running %d pods is more than expected %d, So deleting %d pods on nodeLabel %s", flow, serviceInstanceNum, numOfExistingPods, replicas, numOfPodsToBeDeleted, nodeLabel)
							DeleteExtraPods(nameSpace, ExistingPodsList, numOfPodsToBeDeleted)
						}
					}

				} else {
					glog.Infof("flow=%s serviceInstanceNum=%d GetNumOfExistingPods failed. Ignoring Custom scheduling for nodeLabel=%s", flow, serviceInstanceNum, nodeLabel)
				}
			}
		} else {
...
func RunBatchJobForPodsCleanup() {
	...
	for {
		...
		for _, ns := range namespcaeData.Items {
			...
			if ok := isNamespaceAllowed(ns.Name); ok {
