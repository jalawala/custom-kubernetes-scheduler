...
export NAMESPACE="custom-kube-scheduler-test"
...
L1=$(kubectl get pods -n $NAMESPACE -l app=alpine -o jsonpath='{.items[*].spec.nodeSelector}' | grep  -o  $LABEL1 | wc -l)
echo "Number of Occurences for $LABEL1 is $L1"
L2=$(kubectl get pods -n $NAMESPACE -l app=alpine -o jsonpath='{.items[*].spec.nodeSelector}' | grep  -o  $LABEL2 | wc -l)
echo "Number of Occurences for $LABEL2 is $L2"
L3=$(kubectl get pods -n $NAMESPACE -l app=alpine -o jsonpath='{.items[*].spec.nodeSelector}' | grep  -o  $LABEL3 | wc -l)
